      ...
      const statusConnectionMeta = document.getElementById("status-connection-meta");
      const statusAuth = document.getElementById("status-auth");
      const statusUser = document.getElementById("status-user");
      const statusSession = document.getElementById("status-session");
      const statusSessionMeta = document.getElementById("status-session-meta");
      const statusLimits = document.getElementById("status-limits");
      const statusLimitsMeta = document.getElementById("status-limits-meta");
      ...
      const antiSpamHint = document.getElementById("anti-spam-hint");
      ...
      const contactTimezone = document.getElementById("contact-timezone");
      const profileResult = document.getElementById("profile-result");
      const historyTotal = document.getElementById("history-total");
      const historyInbound = document.getElementById("history-inbound");
      const historyOutbound = document.getElementById("history-outbound");
      const historyLastIn = document.getElementById("history-last-in");
      const historyLastOut = document.getElementById("history-last-out");
      ...
      const eventLabels = {
        ...
        auth_request_code: "Запрос кода",
        auth_submit_code: "Подтверждение кода",
        auth_submit_password: "Подтверждение 2FA",
        auth_logout: "Выход из аккаунта",
        chat_mark_read: "Чат отмечен прочитанным",
        chat_profile_saved: "Профиль чата сохранен"
      };
      ...
      function isChatNew(chat) {
        if (!chat || !chat.last_timestamp) return false;
        const last = new Date(chat.last_timestamp);
        if (Number.isNaN(last.getTime())) return false;
        const now = Date.now();
        return (now - last.getTime()) < 24 * 60 * 60 * 1000;
      }
      ...
      function toggleRetry(show) {
        retryButton.style.display = show ? "inline-flex" : "none";
      }
      ...
      function renderChats() {
        ...
        const filtered = state.chats.filter(chat => {
          ...
          const matchesTerm = !term || name.includes(term) || username.includes(term);
          if (!matchesTerm) return false;

          if (state.chatFilter === "unread") {
            return chat.unread_count && chat.unread_count > 0;
          }
          if (state.chatFilter === "new") {
            return isChatNew(chat);
          }
          if (state.chatFilter === "errors") {
            return false;
          }
          return true;
        });
        ...
        filtered.forEach(chat => {
          ...
          const displayName = chat.display_name || chat.username || ("User " + chat.chat_id);
          ...
          time.textContent = formatTime(chat.last_timestamp);
          ...
          preview.textContent = chat.last_message ? chat.last_message.slice(0, 36) : "Нет сообщений";
        ...
        emptyChats.style.display = filtered.length ? "none" : "block";
        chatCount.textContent = state.chats.length.toString();
      }
      ...
      function renderMessages(messages) {
        ...
        messages.forEach(msg => {
          ...
          const meta = document.createElement("div");
          meta.className = "meta";
          const directionLabel = msg.direction === "inbound" ? "IN" : "OUT";
          meta.textContent = `${directionLabel} • ${msg.display_name || msg.username || msg.chat_id} • ${formatDateTime(msg.timestamp)}`;
          if (msg.direction === "outbound" && msg.status) {
            const badge = document.createElement("span");
            badge.className = `status-pill status-${msg.status}`;
          ...
          if (msg.media_url) {
            ...
            link.target = "_blank";
            link.rel = "noopener";
            link.textContent = msg.media_name || "media";
      ...
      function setContactEmpty() {
        contactEmpty.style.display = "block";
        contactBody.style.display = "none";
        contactName.textContent = "--";
        contactUsername.textContent = "--";
        contactId.textContent = "--";
        contactPhone.textContent = "--";
        contactTags.value = "";
        contactNotes.value = "";
        contactConsent.checked = false;
        contactOptout.checked = false;
        quietStart.value = "";
        quietEnd.value = "";
        contactTimezone.value = "UTC";
        profileResult.textContent = "";
        historyTotal.textContent = "0";
        historyInbound.textContent = "0";
        historyOutbound.textContent = "0";
        historyLastIn.textContent = "--";
        historyLastOut.textContent = "--";
      ...
      async function refreshStatus() {
        try {
          ...
          statusConnection.textContent = "Ошибка";
          statusAuth.textContent = "Ошибка";
          accountStatus.textContent = "Ошибка";
        }
      }

      function updateOnboarding() {
        const showAuth = !state.authorized;
        const showEmpty = state.authorized && state.chats.length === 0;
        if (showAuth || showEmpty) {
          onboarding.style.display = "grid";
          onboardingEmpty.style.display = showEmpty ? "block" : "none";
        } else {
      ...
      async function loadMessages() {
        ...
        if (!state.activeChatId) {
          ...
          activeMeta.textContent = "Нет активного диалога";
          sendHint.textContent = "Выберите чат, чтобы ответить.";
          return;
        }
        try {
      ...
      async function loadChatDetails(chatId) {
        ...
        try {
          ...
          contactUsername.textContent = chat.username ? "@" + chat.username : "--";
          contactId.textContent = chat.chat_id || chatId;
          contactPhone.textContent = chat.phone || "--";
          contactTags.value = (data.profile && data.profile.tags) || "";
          contactNotes.value = (data.profile && data.profile.notes) || "";
          contactConsent.checked = Boolean(data.profile && data.profile.has_consent);
          contactOptout.checked = Boolean(data.profile && data.profile.opted_out);
          quietStart.value = (data.profile && data.profile.quiet_hours_start) || "";
          quietEnd.value = (data.profile && data.profile.quiet_hours_end) || "";
          contactTimezone.value = (data.profile && data.profile.timezone) || "UTC";
          profileResult.textContent = "";

          const history = data.history || {};
          historyTotal.textContent = history.messages_total || 0;
          historyInbound.textContent = history.inbound_count || 0;
          historyOutbound.textContent = history.outbound_count || 0;
          historyLastIn.textContent = formatDateTime(history.last_inbound_at);
          historyLastOut.textContent = formatDateTime(history.last_outbound_at);

          recentList.innerHTML = "";
          (data.recent_messages || []).forEach(msg => {
            const row = document.createElement("div");
            row.textContent = `${msg.direction === "inbound" ? "IN" : "OUT"} • ${formatTime(msg.timestamp)} • ${msg.text}`;
      ...
      async function saveProfile() {
        ...
        const res = await fetch(`/api/ui/chat/${state.activeChatId}/profile?account_id=${state.activeAccountId}`, {
          ...
          body: JSON.stringify({
            ...
            has_consent: contactConsent.checked,
            opted_out: contactOptout.checked,
            quiet_hours_start: quietStart.value.trim(),
            quiet_hours_end: quietEnd.value.trim(),
            timezone: contactTimezone.value.trim()
          })
        });
        const data = await res.json();
        setResult(profileResult, data.success, data.success ? "Сохранено" : extractMessage(data) || "Ошибка");
        loadEvents();
      }
      ...
      async function sendToSelected() {
        ...
        const retryable = data.retryable || (data.detail && data.detail.retryable);
        setResult(sendResult, ok, extractMessage(data) || "Ошибка");
        if (!ok && retryable) {
      ...
      async function sendNewChat() {
        ...
        const ok = data.success === true;
        const retryable = data.retryable || (data.detail && data.detail.retryable);
        setResult(newResult, ok, extractMessage(data) || "Ошибка");
        if (!ok && retryable) {
          state.lastFailedPayload = payload;
          toggleRetry(true);
        } else {
        ...
        if (ok) {
          ...
          if (data.chat && data.chat.chat_id) {
            await selectChat(data.chat.chat_id);
          } else {
            await loadChats();
          }
        }
      ...
      async function retryLast() {
        ...
        const res = await fetch("/api/ui/send", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(state.lastFailedPayload)
        });
        const data = await res.json();
        const ok = data.success === true;
        setResult(sendResult, ok, extractMessage(data) || "Ошибка");
      ...
      async function loadEvents() {
        try {
          const res = await fetch("/api/ui/events?limit=60");
          const data = await res.json();
      ...
      function renderEvents(events) {
        ...
        slice.forEach(event => {
          ...
          const meta = document.createElement("div");
          meta.className = "meta";
          meta.textContent = `${formatTime(event.timestamp)} • ${event.level}`;
          const body = document.createElement("div");
          const label = eventLabels[event.message] || event.message;
          const detail = event.data && event.data.chat_id ? ` (chat ${event.data.chat_id})` : "";
      ...
      function startStream() {
        ...
        stream.addEventListener("ui_message", async (event) => {
          ...
          if (payload.chat_id) {
            await loadChats();
            if (state.activeChatId === payload.chat_id) {
              await loadMessages();
            }
          }
        });
        stream.addEventListener("ui_event", async () => {
          await loadEvents();
        });
        stream.onerror = () => {
          stream.close();
      ...
      document.getElementById("btn-send").addEventListener("click", sendToSelected);
      document.getElementById("btn-send-new").addEventListener("click", sendNewChat);
      document.getElementById("btn-retry").addEventListener("click", retryLast);
      document.getElementById("btn-logout").addEventListener("click", logout);
      document.getElementById("btn-save-profile").addEventListener("click", saveProfile);
      document.getElementById("btn-refresh-events").addEventListener("click", loadEvents);
      document.getElementById("btn-refresh-contact").addEventListener("click", () => {
        if (state.activeChatId) {
          loadChatDetails(state.activeChatId);
        }
      });
      ...
      function setAccountsCollapsed(collapsed) {
        state.accountsCollapsed = collapsed;
        document.body.classList.toggle("accounts-collapsed", collapsed);
        if (collapseButton) {
          const icon = collapseButton.querySelector(".material-icons-outlined");
          if (icon) {
            icon.textContent = collapsed ? "chevron_right" : "chevron_left";
          }
        }
        try {
          localStorage.setItem("accountsCollapsed", collapsed ? "1" : "0");
        } catch (err) {
          // ignore
        }
      }
      ...
      if (collapseButton) {
        collapseButton.addEventListener("click", () => {
          setAccountsCollapsed(!state.accountsCollapsed);
        });
        try {
          const saved = localStorage.getItem("accountsCollapsed");
          if (saved === "1") {
      ...
      if (chatFilters) {
        chatFilters.querySelectorAll("button").forEach(button => {
          button.addEventListener("click", () => {
            const filter = button.dataset.filter || "all";
            state.chatFilter = filter;
            chatFilters.querySelectorAll("button").forEach(item => {
      ...
      function bindStatusToggle() {
        if (!statusToggle || !statusPanel) return;
        ...
        document.addEventListener("click", (event) => {
          ...
          if (statusPanel.contains(event.target) || statusToggle.contains(event.target)) {
            return;
          }
      ...
      document.addEventListener("keydown", (event) => {
        ...
        if (metaKey && event.shiftKey && event.key.toLowerCase() === "n") {
